...
public:
  ...
  /*Map()
  {
      BinarySearchTree<Pair_type, PairComp> *mapTree =
          new BinarySearchTree<Pair_type, PairComp>;
      tree = mapTree;
  }
  ...
  Map(const Map &rhs)
  {
      tree = new BinarySearchTree<Pair_type, PairComp>;
      *tree = *(rhs.tree);
  }
  ...
  Iterator find(const Key_type& k) const
  {
      ...
      findK.first = k;
      findK.second = Value_type();

      return tree.find(findK);
  }
  ...
  Value_type& operator[](const Key_type& k)
 {
  ...
  std::pair<Iterator, bool> insert(const Pair_type& val)
  {
      ...
      {
          returnPair.first = find(val.first);
          returnPair.second = false;
          return returnPair;
      }
      else
      {
          //Inserting val, but not sure if its right
          tree.insert(val);
          returnPair.first = find(val.first);
