...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) 
  {
      ...
      int heightRight = height_impl(node->right);
      int heightLeft = height_impl(node->left);
 ...
 // EFFECTS :Searches the tree rooted at 'node' for an element equivalent
  ...
  static Node* min_greater_than_impl(Node* node, const T& val, Compare less)
  {
     ...
     {
         Node* result = min_greater_than_impl(node->right, val, less);
         return result;
     }
     ...
     if (!less(val, node->datum) && !less(node->datum, val) && node->right != nullptr)
     {
         return node->right;
     }
     else
