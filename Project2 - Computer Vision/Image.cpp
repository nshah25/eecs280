...
void Image_init(Image* img, std::istream& is) 
{
	...
	//int* r = Matrix_at(&img->red_channel, 0, 0);
	//int* g= Matrix_at(&img->green_channel, 0, 0);
	//sint* b = Matrix_at(&img->blue_channel, 0, 0);
	for (int row = 0; row < img->height; row++)
	{
		for (int col = 0; col < img->width; col++)
		{
			is >> *Matrix_at(&img->red_channel, row, col) >> 
			*Matrix_at(&img->green_channel, row, col) >> *Matrix_at(&img->blue_channel, row, col);
		}
	}
